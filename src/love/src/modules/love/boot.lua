...
function love.init()
	...
	--? process DO_NOT_LOAD
	for i = 1, #DO_NOT_LOAD, 1 do
		if c.modules[DO_NOT_LOAD[i]] == true then
			c.modules[DO_NOT_LOAD[i]] = false
			print(DO_NOT_LOAD[i].." not loaded")
		end
	end
	...
	--? process DO_NOT_LOAD
	for i = 1, #DO_NOT_LOAD, 1 do
		for f = 1, #to_load, 1 do
			if DO_NOT_LOAD[i] == to_load[f] then
				table.remove(to_load, f)
			end
		end
	end
	for i = 1, #to_load, 1 do
		require("love."..to_load[i])
	end
